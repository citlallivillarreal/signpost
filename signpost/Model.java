...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int x = 0; x < _width; x += 1) {
            for (int y = 0; y < _height; y += 1) {
                ...
                _solnNumToPlace[seqNum] = Place.pl(x, y); }
        } _board = new Sq [_width][_height];
        for (int x = 0; x < _width; x += 1) {
            for (int y = 0; y < _height; y += 1) {
                int dir = arrowDirection(x, y);
                if (_solution[x][y] == 1) {
                    _board[x][y] = new Sq(x, y, 1, true, dir, 0);
                    _allSquares.add(_board[x][y]);
                } else if (_solution[x][y] == _height * _width) {
                    _board[x][y] = new Sq(x, y, last, true, 0, 0);
                    _allSquares.add(_board[x][y]);
                } else {
                    _board[x][y] = new Sq(x, y, 0, false, dir, -1);
                    _allSquares.add(_board[x][y]);
                }
            }
        }
        for (int counter = 1; counter < last; counter += 1) {
            if (_solnNumToPlace[counter] == null) {
                throw badArgs("must have at least 2 squares");
            }
        }
        for (Sq[] col: _board) {
            for (Sq sq : col) {
                int dir = arrowDirection(sq.x, sq.y);
                sq._successors = allSuccessors(sq.x, sq.y, dir);
            }
        }
        for (Sq[] col: _board) {
            for (Sq sq : col) {
                ...
                for (int x0 = 0; x0 < _width; x0 += 1) {
                    for (int y0 = 0; y0 < _height; y0 += 1) {
                        int dir = _board[x0][y0]._dir;
                        for (Place p : _allSuccessors[x0][y0][dir]) {
                            if (p.equals(sq.pl)) {
                                sq._predecessors.add(p);
                            }
                        }
                    }
                }
    ...
    Model(Model model) {
        ...
        for (int x = 0; x < _width; x += 1) {
            for (int y = 0; y < _height; y += 1) {
                _allSquares.add(model._board[x][y]);
            }
        }
        for (int x = 0; x < _width; x += 1) {
            for (int y = 0; y < _height; y += 1) {
                ...
                if (_solution[x][y] != 1 && _solution[x][y] != last) {
                    int xsuccessor = _solnNumToPlace[_solution[x][y] + 1].x;
                    int ysuccessor = _solnNumToPlace[_solution[x][y] + 1].y;
                    int xpred = _solnNumToPlace[_solution[x][y] - 1].x;
                    int ypred = _solnNumToPlace[_solution[x][y] - 1].y;
                    _board[x][y]._successor = _board[xsuccessor][ysuccessor];
                    _board[x][y]._predecessor = _board[xpred][ypred];
    ...
    void init(int width, int height) {
        ...
        _board = new Sq [_width][_height];
        _allSquares = new ArrayList<>(_width * _height);
        ...
        for (int x = 0; x < _width; x += 1) {
            for (int y = 0; y < _height; y += 1) {
                ...
                if (sq._successor == null
                        && sq._successors.size() != 0
                        && sq._sequenceNum != 0) {
                    int x0 = 0;
                    PlaceList sqpl = sq._successors;
                    for (; x0 < sqpl.size(); x0 += 1) {
                        if (sq.connectable(get(sqpl.get(x0)))
                                && get(sqpl.get(x0))._sequenceNum != 0) {
                            sq.connect(get(sqpl.get(x0)));
                            flag = true;
                        }
                    }
                }
    ...
    void solve() {
        for (int x = 0; x < _width; x += 1) {
            for (int y = 0; y < _height; y += 1) {
                int sqNum = _solution[x][y];
                _board[x][y]._sequenceNum = sqNum;
            }
        }
        autoconnect();
        _unconnected = 0;
    ...
    private int arrowDirection(int x, int y) {
        ...
        if (_solution[x][y] == _width * _height) {
            return 0;
        }
        Place succpl = _solnNumToPlace[_solution[x][y] + 1];
        return Place.dirOf(x, y, succpl.x, succpl.y);
    }
    ...
    final class Sq {
         ...
         *  + S1 is in the correct direction from me.*Done*
         *  + S1 does not have a current predecessor, and I do not have a
         *    current successor. *Done*
         *  + If S1 and I both have sequence numbers, then mine is
         *    sequenceNum() == S1.sequenceNum() - 1. *Done*
         *  + If neither S1 nor I have sequence numbers, then we are not part
         *    of the same connected sequence. *Done*
         */
        boolean connectable(Sq s1) {
            if (this._successors.contains(s1.pl) && s1._predecessor == null
        ...
        boolean connect(Sq s1) {
            ...
            if (this._sequenceNum != 0 && s1._sequenceNum == 0) {
                releaseGroup(sgroup);
                Sq succ = s1;
                int seqsucc = this._sequenceNum;
                while (succ != null) {
                    succ._sequenceNum = seqsucc + 1;
                    seqsucc += 1;
                ...
                int seq = s1._sequenceNum;
                while (pred != null) {
                    pred._sequenceNum = seq - 1;
                    pred = pred._predecessor;
                    seq -= 1;
                }
            } else if (s1._sequenceNum == 0 && this._sequenceNum == 0) {
                this.head()._group = joinGroups(this.group(), sgroup);
            }
            Sq next = s1;
            while (next != null) {
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (this._predecessor == null && next._successor == null) {
                    ...
                    next._group = this._group; next._group = -1;
                } else {
                ...
                if (flag) {
                    ...
                    if (this._predecessor != null) {
                        ...
                        while (pred != null) {
                            pred._sequenceNum = 0;
                            pred._group = this._group;
                            pred = pred._predecessor; }
                    } else {
                        this._sequenceNum = 0; this._group = -1;
                    }
                }
                Sq succ = next; boolean check = false;
                while (succ != null) {
                    if (succ.hasFixedNum()) {
                        check = true; break;
                    }
                ...
                if (check) {
                    ...
                    if (next._successor != null) {
                        next._group = newGroup();
                        while (succ != null) {
                            succ._sequenceNum = 0; succ._group = next._group;
                            succ = succ._successor; }
                    } else {
                        next._sequenceNum = 0; next._group = -1;
                    }
                }
                next._head = next; Sq nextsuccs = next._successor;
                while (nextsuccs != null) {
                    nextsuccs._head = next; nextsuccs = nextsuccs._successor;
                }
            }
        }
        @Override
